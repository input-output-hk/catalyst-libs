...
mod common;
use common::parser::{check_tests_rule, Rule};

/// Test if the `WHITESPACE` rule passes properly.
#[test]
fn check_whitespace() {
    check_tests_rule(Rule::WHITESPACE, &[" ", "\t", "\r", "\n", "\r\n"], &["not"]);
}
...
fn check_pchar() {
    ...
    let fails = ('\u{0}'..='\u{ff}')
        ...
        .map(String::from)
        .collect::<Vec<_>>();
    check_tests_rule(Rule::PCHAR, &passes, &fails);
}
...
fn check_bchar() {
    ...
    check_tests_rule(Rule::BCHAR, &passes, &fails);
}
...
fn check_sesc() {
    let passes = (' '..='\u{ff}')
        ...
        .map(|x| format!("\\{x}"))
        .collect::<Vec<_>>();
    check_tests_rule(Rule::SESC, &passes, &["\u{7f}"]);
}
...
fn check_ascii_visible() {
    ...
    check_tests_rule(Rule::ASCII_VISIBLE, &passes, &["\r", "\u{80}"]);
}
...
fn check_schar_ascii_visible() {
    let passes = (' '..='~')
        ...
        .map(String::from)
        .collect::<Vec<_>>();
    check_tests_rule(Rule::SCHAR_ASCII_VISIBLE, &passes, &[
        "\"", "\\", "\r", "\u{80}",
    ]);
}
...
fn check_bchar_ascii_visible() {
    let passes = (' '..='~')
        ...
        .map(String::from)
        .collect::<Vec<_>>();
    check_tests_rule(Rule::BCHAR_ASCII_VISIBLE, &passes, &[
        "'", "\\", "\r", "\u{80}",
    ]);
}
