...
mod common;
use common::parser::{check_tests_rule, type_declarations::*, Rule};

/// Test if the `ctlop` rule passes properly.
/// This uses a special rule in the Grammar to test `ctlop` exhaustively.
#[test]
fn check_ctlop() {
    check_tests_rule(Rule::ctlop_TEST, CTLOP_PASSES, CTLOP_FAILS);
}
...
#[test]
fn check_rangeop() {
    check_tests_rule(Rule::rangeop_TEST, RANGEOP_PASSES, RANGEOP_FAILS);
}
...
#[test]
fn check_type2() {
    check_tests_rule(Rule::type2_TEST, TYPE2_PASSES, TYPE2_FAILS);
}
...
#[test]
fn check_type1() {
    check_tests_rule(Rule::type1_TEST, TYPE1_PASSES, TYPE1_FAILS);
}
...
fn check_type1_composition() {
    ...
    check_tests_rule(Rule::type1_TEST, &passes, &fails);
}
...
#[test]
fn check_type() {
    check_tests_rule(Rule::type_TEST, TYPE_PASSES, TYPE_FAILS);
}
...
fn check_type_composition() {
    ...
    check_tests_rule(Rule::type_TEST, &passes, &fails);
}
