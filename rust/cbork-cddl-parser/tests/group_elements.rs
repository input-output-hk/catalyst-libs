...
mod common;
use common::parser::{check_tests_rule, group_elements::*, identifiers::*, Rule};

#[test]
/// Test if the `occur` rule passes properly.
/// This uses a special rule in the Grammar to test `occur` exhaustively.
fn check_occur() {
    check_tests_rule(Rule::occur_TEST, OCCUR_PASSES, OCCUR_FAILS);
}
...
fn check_bareword() {
    check_tests_rule(Rule::bareword_TEST, ID_PASSES, ID_FAILS);
}
...
fn check_optcom() {
    check_tests_rule(Rule::optcom_TEST, OPTCOM_PASSES, OPTCOM_FAILS);
}
...
fn check_memberkey() {
    check_tests_rule(Rule::memberkey_TEST, MEMBERKEY_PASSES, MEMBERKEY_FAILS);
}
...
fn check_grpent() {
    check_tests_rule(Rule::grpent_TEST, GRPENT_PASSES, GRPENT_FAILS);
}
...
fn check_grpchoice() {
    check_tests_rule(Rule::grpchoice_TEST, GRPCHOICE_PASSES, GRPCHOICE_FAILS);
}
