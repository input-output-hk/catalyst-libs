...
mod common;
use common::parser::{check_tests_rule, rules::*, type_declarations::*, Rule};

/// Test if the `genericarg` rule passes properly.
/// This uses a special rule in the Grammar to test `genericarg` exhaustively.
#[test]
fn check_genericarg() {
    check_tests_rule(Rule::genericarg_TEST, GENERICARG_PASSES, GENERICARG_FAILS);
}
...
#[test]
fn check_assigng() {
    check_tests_rule(Rule::assigng_TEST, ASSIGNG_PASSES, ASSIGNG_FAILS);
}
...
#[test]
fn check_assignt() {
    check_tests_rule(Rule::assignt_TEST, ASSIGNT_PASSES, ASSIGNT_FAILS);
}
...
#[test]
fn check_typename() {
    check_tests_rule(Rule::typename_TEST, TYPENAME_PASSES, TYPENAME_FAILS);
}
...
#[test]
fn check_groupname() {
    check_tests_rule(Rule::groupname_TEST, GROUPNAME_PASSES, GROUPNAME_FAILS);
}
...
fn check_rule_type_composition() {
    ...
    check_tests_rule(Rule::expr_TEST, &passes, &fails);
}

/// Test if the `rule` rule passes properly for group variant.
#[test]
fn check_rule_group() {
    check_tests_rule(Rule::expr_TEST, RULE_GROUP_PASSES, RULE_GROUP_FAILS);
}
