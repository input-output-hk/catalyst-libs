...
//
// Returns a height value between 1 and `MAX_HEIGHT` that provides reasonable batching for
// the given count.
pub(super) fn coarse_height(count: usize) -> u8 {
    let d = ((count as f64) / 64.0).max(1.0).log2();
    let d = d.max(1.0).min(f64::from(MAX_HEIGHT));
    d.round() as u8
}
...
mod tests {
    ...
    #[test_case(370727 => 12)]
    #[test_case(370728 => 13)]
    #[test_case(741456 => MAX_HEIGHT)]
    #[test_case(usize::MAX => MAX_HEIGHT; "should be capped at 14")]
