...
async fn single_signature_validation_test() {
    let (signed_doc, pk, kid) = common::create_dummy_signed_doc(None, RoleIndex::Role0).unwrap();
...
async fn multiple_signatures_validation_test() {
    let (sk1, pk1, kid1) = common::create_dummy_key_pair(RoleIndex::Role0).unwrap();
    let (sk2, pk2, kid2) = common::create_dummy_key_pair(RoleIndex::Role0).unwrap();
    let (sk3, pk3, kid3) = common::create_dummy_key_pair(RoleIndex::Role0).unwrap();
    let (_, pk_n, kid_n) = common::create_dummy_key_pair(RoleIndex::Role0).unwrap();
