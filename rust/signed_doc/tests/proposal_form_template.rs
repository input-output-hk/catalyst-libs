...
//! <https://input-output-hk.github.io/catalyst-libs/architecture/08_concepts/signed_doc/docs/proposal_form_template>
...
// #[test_case(
//     |provider| {
//         let parameters = brand_parameters_doc().inspect(|v| provider.add_document(None,
// v).unwrap())?;         let template =
// proposal_form_template_doc(&parameters).inspect(|v| provider.add_document(None,
// v).unwrap())?;         let id = UuidV7::new();         let (sk, _, kid) =
// create_dummy_key_pair(RoleId::Role0).inspect(|(_, pk, kid)| provider.add_pk(kid.
// clone(), *pk))?;         Builder::new()
// .with_json_metadata(serde_json::json!({                 "content-type":
// ContentType::Json.to_string(),                 "content-encoding":
// ContentEncoding::Brotli.to_string(),                 "type":
// doc_types::PROPOSAL.clone(),                 "id": id,
//                 "ver": id,
//                 "template": {
//                     "id": template.doc_id()?,
...
// proposal_form_template_doc(&parameters).inspect(|v| provider.add_document(None,
// v).unwrap())?;         let id = UuidV7::new();         let (sk, _, kid) =
// create_dummy_key_pair(RoleId::Proposer).inspect(|(_, pk, kid)| provider.add_pk(kid.
// clone(), *pk))?;         Builder::new()
// .with_json_metadata(serde_json::json!({                 "content-type":
// ContentType::Json.to_string(),                 "content-encoding":
// ContentEncoding::Brotli.to_string(),                 "type":
...
// clone(), *pk))?;         Builder::new()
// .with_json_metadata(serde_json::json!({                 "content-type":
// ContentType::Json.to_string(),                 "type": doc_types::PROPOSAL.clone(),
//                 "id": id,
//                 "ver": id,
//                 "template": {
//                     "id": template.doc_id()?,
...
//     |provider| {
//         let parameters = brand_parameters_doc().inspect(|v| provider.add_document(None,
// v).unwrap())?;         let id = UuidV7::new();
//         let (sk, _, kid) = create_dummy_key_pair(RoleId::Proposer).inspect(|(_, pk,
// kid)| provider.add_pk(kid.clone(), *pk))?;         Builder::new()
