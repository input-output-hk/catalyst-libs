...
/// A mapping from certificate index to
type UrisMap = HashMap<usize, Cip0134Uri>;

/// A set of [`Cip0134Uri`] contained in both x509 and c509 certificates stored in the
/// metadata part of [`Cip509`].
///
...
//             Ok((_, der)) => der,
//             Err(e) => {
//                 debug!("Failed to parse DER sequence for Subject Alternative Name
// ({extension:?}): {e:?}");                 continue;
//             },
//         };
...
//                 continue;
//             };
//             let GeneralNamesOrText::GeneralNames(gen_names) = alt_name.general_name()
// else {                 debug!("Unexpected general name type: {extension:?}");
//                 continue;
//             };
//             for name in gen_names.general_names() {
//                 if *name.gn_type() !=
// GeneralNameTypeRegistry::UniformResourceIdentifier {                     continue;
//                 }
...
mod tests {
    ...
    // // `get(n).unwrap()` instead because Clippy will still complain
    // (clippy::get-unwrap). #[allow(clippy::indexing_slicing)]
    // #[test]
    // fn list_new() {
    //     let block =
    //         hex::decode(include_str!("../../../../test_data/cardano/conway_1.block")).
    // unwrap();     let block = MultiEraBlock::decode(&block).unwrap();
    //     let tx = &block.txs()[3];
